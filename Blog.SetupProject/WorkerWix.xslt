...
>
  ...
  <xsl:template match="wix:Wix">
    ...
    <Wix>
      <Fragment>
        <DirectoryRef Id="Worker.Output.Content">
          <xsl:apply-templates select="$content"/>
        </DirectoryRef>

        <DirectoryRef Id="Worker.Output.Binaries">
          <xsl:apply-templates select="$binaries"/>
        </DirectoryRef>
      </Fragment>

      <Fragment>
        <ComponentGroup Id="Worker.Output.Content">
          <xsl:for-each select="$content">
            <ComponentRef Id="{concat('w',@Id)}"/>
          </xsl:for-each>
        </ComponentGroup>
        <ComponentGroup Id="Worker.Output.Binaries">
  ...
  <xsl:template match="wix:Component">
    <Component Id="{concat('w',@Id)}" Guid="{@Guid}">
      <File Id="{concat('w',wix:File/@Id)}" Source="{concat('$(var.Blog.Worker.TargetDir)', substring(wix:File/@Source, 11))}" Name="{substring(wix:File/@Source, 11)}" />
    </Component>
  </xsl:template>
